...
def main():
    seq_path = output_dir / f"sequences_{output_size}.jsonl"  
    assert seq_path.exists() and seq_path.stat().st_size > 0, f"Missing/empty: {seq_path}"

    with seq_path.open("r", encoding="utf-8") as f:
        checkpoints_results = pd.read_json(f, orient="records", lines=True)
